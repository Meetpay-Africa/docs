...
`details` is optional. For validation failures it is an array of field-level errors (`[{field, message}]`, e.g. `{"field": "customer.email", "message": "must be a valid email"}`); for payment outcomes it carries context such as `transaction_id` or `existing_transaction_id`.
...
| 401 | `invalid_api_key` | API key invalid, revoked, or inactive |
...
| HTTP | `error_code` | Meaning |
| :--- | :--- | :--- |
| 403 | `TEST_MODE_UNAVAILABLE` | `sk_test_*` key on a money-moving endpoint while test mode is not enabled for your account. Test keys never move real money; payouts are always blocked for test keys |
